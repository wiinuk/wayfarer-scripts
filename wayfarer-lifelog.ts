...
const parsePropertiesResponse = (response: string) => {
    ...
    const email =
        asStringOrNull(result.socialProfile?.email) ??
        error`Unexpected response`;
    const performance =
        asStringOrNull(result.performance) ?? error`Unexpected response`;
    const rewardProgress =
        asNumberOrNull(result.rewardProgress) ?? error`Unexpected response`;
...
let insertedGraphElement: HTMLElement | null = null;
const getInsertedGraphElement = async () => {
    if (insertedGraphElement != null) {
        return insertedGraphElement;
    }
    ...
    return (insertedGraphElement = graphElement);
};
// プロパティを要求したとき
const onGetProperties = async function (this: XMLHttpRequest) {
    ...
    const graphElement = await getInsertedGraphElement();
};
